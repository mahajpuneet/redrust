...
#[no_mangle]
pub unsafe extern "C" fn slowlogCreateEntry(
    mut c: *mut client,
    mut argv: *mut *mut robj,
    mut argc: libc::c_int,
    mut duration: libc::c_longlong,
) -> *mut slowlogEntry {
    let mut se: *mut slowlogEntry = zmalloc(
        core::mem::size_of::<slowlogEntry>() as libc::c_ulong,
    ) as *mut slowlogEntry;
    let mut j: libc::c_int = 0;
    let mut slargc: libc::c_int = argc;
    if slargc > 32 as libc::c_int {
        slargc = 32 as libc::c_int;
    }
    (*se).argc = slargc;
    (*se)
        .argv = zmalloc(
        (core::mem::size_of::<*mut robj>() as libc::c_ulong)
            .wrapping_mul(slargc as libc::c_ulong),
    ) as *mut *mut robj;
    j = 0 as libc::c_int;
    while j < slargc {
        if slargc != argc && j == slargc - 1 as libc::c_int {
            let ref mut fresh0 = *((*se).argv).offset(j as isize);
            *fresh0 = createObject(
                0 as libc::c_int,
                sdscatprintf(
                    sdsempty(),
                    b"... (%d more arguments)\0" as *const u8 as *const libc::c_char,
                    argc - slargc + 1 as libc::c_int,
                ) as *mut libc::c_void,
            );
        } else if (**argv.offset(j as isize)).type_0() as libc::c_int == 0 as libc::c_int
            && ((**argv.offset(j as isize)).encoding() as libc::c_int == 0 as libc::c_int
                || (**argv.offset(j as isize)).encoding() as libc::c_int
                    == 8 as libc::c_int)
            && sdslen((**argv.offset(j as isize)).ptr as sds)
                > 128 as libc::c_int as libc::c_ulong
        {
            let mut s: sds = sdsnewlen(
                (**argv.offset(j as isize)).ptr,
                128 as libc::c_int as size_t,
            );
            s = sdscatprintf(
                s,
                b"... (%lu more bytes)\0" as *const u8 as *const libc::c_char,
                (sdslen((**argv.offset(j as isize)).ptr as sds))
                    .wrapping_sub(128 as libc::c_int as libc::c_ulong),
            );
            let ref mut fresh1 = *((*se).argv).offset(j as isize);
            *fresh1 = createObject(0 as libc::c_int, s as *mut libc::c_void);
        } else if (**argv.offset(j as isize)).refcount == 2147483647 as libc::c_int {
            let ref mut fresh2 = *((*se).argv).offset(j as isize);
            *fresh2 = *argv.offset(j as isize);
        } else {
            let ref mut fresh3 = *((*se).argv).offset(j as isize);
            *fresh3 = dupStringObject(*argv.offset(j as isize));
        }
        j += 1;
    }
    (*se).time = time(0 as *mut time_t);
    (*se).duration = duration;
    let fresh4 = server.slowlog_entry_id;
    server.slowlog_entry_id = server.slowlog_entry_id + 1;
    (*se).id = fresh4;
    (*se).peerid = sdsnew(getClientPeerId(c));
    (*se)
        .cname = if !((*c).name).is_null() {
        sdsnew((*(*c).name).ptr as *const libc::c_char)
    } else {
        sdsempty()
    };
    return se;
...
#[no_mangle]
pub unsafe extern "C" fn slowlogFreeEntry(mut septr: *mut libc::c_void) {
    let mut se: *mut slowlogEntry = septr as *mut slowlogEntry;
    let mut j: libc::c_int = 0;
    j = 0 as libc::c_int;
    while j < (*se).argc {
        decrRefCount(*((*se).argv).offset(j as isize));
        j += 1;
    }
    zfree((*se).argv as *mut libc::c_void);
    sdsfree((*se).peerid);
    sdsfree((*se).cname);
    zfree(se as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn slowlogInit() {
    server.slowlog = listCreate();
    server.slowlog_entry_id = 0 as libc::c_int as libc::c_longlong;
    (*server.slowlog)
        .free = Some(slowlogFreeEntry as unsafe extern "C" fn(*mut libc::c_void) -> ());
}
#[no_mangle]
pub unsafe extern "C" fn slowlogPushEntryIfNeeded(
    mut c: *mut client,
    mut argv: *mut *mut robj,
    mut argc: libc::c_int,
    mut duration: libc::c_longlong,
    ...
    if duration >= server.slowlog_log_slower_than {
        listAddNodeHead(
            server.slowlog,
            slowlogCreateEntry(c, argv, argc, duration) as *mut libc::c_void,
        );
    }
...
#[no_mangle]
pub unsafe extern "C" fn slowlogReset() {
    while (*server.slowlog).len > 0 as libc::c_int as libc::c_ulong {
        listDelNode(server.slowlog, (*server.slowlog).tail);
    }
}
#[no_mangle]
pub unsafe extern "C" fn slowlogCommand(mut c: *mut client) {
    ...
    {
        let mut help: [*const libc::c_char; 10] = [
            b"GET [<count>]\0" as *const u8 as *const libc::c_char,
            b"    Return top <count> entries from the slowlog (default: 10, -1 mean all).\0"
                as *const u8 as *const libc::c_char,
            b"    Entries are made of:\0" as *const u8 as *const libc::c_char,
            b"    id, timestamp, time in microseconds, arguments array, client IP and port,\0"
                as *const u8 as *const libc::c_char,
            b"    client name\0" as *const u8 as *const libc::c_char,
            b"LEN\0" as *const u8 as *const libc::c_char,
            b"    Return the length of the slowlog.\0" as *const u8
                as *const libc::c_char,
            b"RESET\0" as *const u8 as *const libc::c_char,
            b"    Reset the slowlog.\0" as *const u8 as *const libc::c_char,
    ...
    {
        addReplyLongLong(c, (*server.slowlog).len as libc::c_longlong);
    } else if ((*c).argc == 2 as libc::c_int || (*c).argc == 3 as libc::c_int)
        && strcasecmp(
            (**((*c).argv).offset(1 as libc::c_int as isize)).ptr as *const libc::c_char,
        ...
        ) == 0
    {
        ...
        let mut sent: libc::c_long = 0 as libc::c_int as libc::c_long;
        let mut li: listIter = listIter {
            next: 0 as *mut listNode,
            direction: 0,
        };
        let mut totentries: *mut libc::c_void = 0 as *mut libc::c_void;
        let mut ln: *mut listNode = 0 as *mut listNode;
        let mut se: *mut slowlogEntry = 0 as *mut slowlogEntry;
        if (*c).argc == 3 as libc::c_int {
            if getRangeLongFromObjectOrReply(
                ...
                *((*c).argv).offset(2 as libc::c_int as isize),
                ...
                &mut count,
                b"count should be greater than or equal to -1\0" as *const u8
                    as *const libc::c_char,
            ) != 0 as libc::c_int
            {
                return;
            ...
            if count == -(1 as libc::c_int) as libc::c_long {
                count = (*server.slowlog).len as libc::c_long;
            }
        }
        listRewind(server.slowlog, &mut li);
        ...
        loop {
            ...
            let mut j: libc::c_int = 0;
            se = (*ln).value as *mut slowlogEntry;
            addReplyArrayLen(c, 6 as libc::c_int as libc::c_long);
            addReplyLongLong(c, (*se).id);
            addReplyLongLong(c, (*se).time as libc::c_longlong);
            addReplyLongLong(c, (*se).duration);
            addReplyArrayLen(c, (*se).argc as libc::c_long);
            j = 0 as libc::c_int;
            while j < (*se).argc {
                addReplyBulk(c, *((*se).argv).offset(j as isize));
                j += 1;
            }
            addReplyBulkCBuffer(
                c,
                (*se).peerid as *const libc::c_void,
                sdslen((*se).peerid),
            );
            addReplyBulkCBuffer(
                c,
                (*se).cname as *const libc::c_void,
                sdslen((*se).cname),
            );
            sent += 1;
        }
