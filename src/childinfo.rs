...
extern "C" {
    ...
    fn zmalloc_get_private_dirty(pid: libc::c_long) -> size_t;
}
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct child_info_data {
    pub keys: size_t,
    pub cow: size_t,
    pub cow_updated: monotime,
    pub progress: libc::c_double,
    pub information_type: childInfoType,
}
#[no_mangle]
pub unsafe extern "C" fn openChildInfoPipe() {
    if anetPipe(
        (server.child_info_pipe).as_mut_ptr(),
...
#[no_mangle]
pub unsafe extern "C" fn closeChildInfoPipe() {
        ...
        || server.child_info_pipe[1 as libc::c_int as usize] != -(1 as libc::c_int)
    {
        ...
        close(server.child_info_pipe[1 as libc::c_int as usize]);
...
#[no_mangle]
pub unsafe extern "C" fn sendChildInfoGeneric(
    mut info_type: childInfoType,
    mut keys: size_t,
    mut progress: libc::c_double,
    mut pname: *mut libc::c_char,
) {
    if server.child_info_pipe[1 as libc::c_int as usize] == -(1 as libc::c_int) {
        return;
    }
    static mut cow_updated: monotime = 0 as libc::c_int as monotime;
    static mut cow_update_cost: uint64_t = 0 as libc::c_int as uint64_t;
    static mut cow: size_t = 0 as libc::c_int as size_t;
    static mut peak_cow: size_t = 0 as libc::c_int as size_t;
    static mut update_count: size_t = 0 as libc::c_int as size_t;
    static mut sum_cow: libc::c_ulonglong = 0 as libc::c_int as libc::c_ulonglong;
    let mut data: child_info_data = {
        let mut init = child_info_data {
            keys: 0 as libc::c_int as size_t,
            cow: 0,
            cow_updated: 0,
            progress: 0.,
            information_type: CHILD_INFO_TYPE_CURRENT_INFO,
        };
        init
    };
    let mut now: monotime = getMonotonicUs.expect("non-null function pointer")();
    if info_type as libc::c_uint
        != CHILD_INFO_TYPE_CURRENT_INFO as libc::c_int as libc::c_uint
        || cow_updated == 0
        || now.wrapping_sub(cow_updated)
            > cow_update_cost.wrapping_mul(100 as libc::c_int as libc::c_ulong)
    {
        cow = zmalloc_get_private_dirty(-(1 as libc::c_int) as libc::c_long);
        cow_updated = getMonotonicUs.expect("non-null function pointer")();
        cow_update_cost = cow_updated.wrapping_sub(now);
        if cow > peak_cow {
            peak_cow = cow;
        }
        sum_cow = sum_cow.wrapping_add(cow as libc::c_ulonglong);
        update_count = update_count.wrapping_add(1);
        let mut cow_info: libc::c_int = (info_type as libc::c_uint
            != CHILD_INFO_TYPE_CURRENT_INFO as libc::c_int as libc::c_uint)
            as libc::c_int;
        if cow != 0 || cow_info != 0 {
            if !(((if cow_info != 0 { 2 as libc::c_int } else { 1 as libc::c_int })
                & 0xff as libc::c_int) < server.verbosity)
            {
                _serverLog(
                    if cow_info != 0 { 2 as libc::c_int } else { 1 as libc::c_int },
                    b"Fork CoW for %s: current %zu MB, peak %zu MB, average %llu MB\0"
                        as *const u8 as *const libc::c_char,
                    pname,
                    cow >> 20 as libc::c_int,
                    peak_cow >> 20 as libc::c_int,
                    sum_cow.wrapping_div(update_count as libc::c_ulonglong)
                        >> 20 as libc::c_int,
                );
            }
        }
    ...
    let mut wlen: ssize_t = core::mem::size_of::<child_info_data>() as libc::c_ulong
        as ssize_t;
    if write(
        server.child_info_pipe[1 as libc::c_int as usize],
        &mut data as *mut child_info_data as *const libc::c_void,
        wlen as size_t,
    ) != wlen
    {
        if !((3 as libc::c_int & 0xff as libc::c_int) < server.verbosity) {
            _serverLog(
                3 as libc::c_int,
                b"Child failed reporting info to parent, exiting. %s\0" as *const u8
...
#[no_mangle]
pub unsafe extern "C" fn updateChildInfo(
    mut information_type: childInfoType,
    mut cow: size_t,
    mut cow_updated: monotime,
    mut keys: size_t,
    mut progress: libc::c_double,
) {
    ...
    if information_type as libc::c_uint
        == CHILD_INFO_TYPE_CURRENT_INFO as libc::c_int as libc::c_uint
    {
        server.stat_current_cow_bytes = cow;
        server.stat_current_cow_updated = cow_updated;
        server.stat_current_save_keys_processed = keys;
        if progress != -(1 as libc::c_int) as libc::c_double {
            server.stat_module_progress = progress;
        }
    } else if information_type as libc::c_uint
        == CHILD_INFO_TYPE_AOF_COW_SIZE as libc::c_int as libc::c_uint
    {
        server.stat_aof_cow_bytes = server.stat_current_cow_peak;
    } else if information_type as libc::c_uint
        == CHILD_INFO_TYPE_RDB_COW_SIZE as libc::c_int as libc::c_uint
    {
        server.stat_rdb_cow_bytes = server.stat_current_cow_peak;
    } else if information_type as libc::c_uint
        == CHILD_INFO_TYPE_MODULE_COW_SIZE as libc::c_int as libc::c_uint
    {
        server.stat_module_cow_bytes = server.stat_current_cow_peak;
    }
}
#[no_mangle]
pub unsafe extern "C" fn readChildInfo(
    mut information_type: *mut childInfoType,
    mut cow: *mut size_t,
    mut cow_updated: *mut monotime,
    mut keys: *mut size_t,
    mut progress: *mut libc::c_double,
) -> libc::c_int {
    static mut buffer: child_info_data = child_info_data {
        keys: 0,
        cow: 0,
        cow_updated: 0,
        progress: 0.,
    ...
    let mut wlen: ssize_t = core::mem::size_of::<child_info_data>() as libc::c_ulong
        as ssize_t;
    if server.child_info_nread as libc::c_long == wlen {
        server.child_info_nread = 0 as libc::c_int;
    }
    let mut nread: libc::c_int = read(
        server.child_info_pipe[0 as libc::c_int as usize],
        (&mut buffer as *mut child_info_data as *mut libc::c_char)
            .offset(server.child_info_nread as isize) as *mut libc::c_void,
        (wlen - server.child_info_nread as libc::c_long) as size_t,
    ) as libc::c_int;
...
#[no_mangle]
pub unsafe extern "C" fn receiveChildInfo() {
    if server.child_info_pipe[0 as libc::c_int as usize] == -(1 as libc::c_int) {
        return;
    }
    let mut cow: size_t = 0;
    let mut cow_updated: monotime = 0;
    let mut keys: size_t = 0;
    let mut progress: libc::c_double = 0.;
    let mut information_type: childInfoType = CHILD_INFO_TYPE_CURRENT_INFO;
    while readChildInfo(
        &mut information_type,
        &mut cow,
        &mut cow_updated,
        &mut keys,
        &mut progress,
