...
extern "C" {
    ...
    fn clusterRedirectBlockedClientIfNeeded(c: *mut client) -> libc::c_int;
    fn ceill(_: f64) -> f64;
}
...
#[no_mangle]
pub unsafe extern "C" fn checkBlockedClientTimeout(
    mut c: *mut client,
    mut now: mstime_t,
) -> libc::c_int {
    if (*c).flags & ((1 as libc::c_int) << 4 as libc::c_int) as libc::c_ulong != 0
        && (*c).bpop.timeout != 0 as libc::c_int as libc::c_longlong
        && (*c).bpop.timeout < now
    {
...
#[no_mangle]
pub unsafe extern "C" fn clientsCronHandleTimeout(
    mut c: *mut client,
    mut now_ms: mstime_t,
) -> libc::c_int {
    let mut now: time_t = (now_ms / 1000 as libc::c_int as libc::c_longlong) as time_t;
    if server.maxidletime != 0
        && (*c).flags & ((1 as libc::c_int) << 0 as libc::c_int) as libc::c_ulong == 0
        && mustObeyClient(c) == 0
        && (*c).flags & ((1 as libc::c_int) << 4 as libc::c_int) as libc::c_ulong == 0
        && (*c).flags & ((1 as libc::c_int) << 18 as libc::c_int) as libc::c_ulong == 0
        && now - (*c).lastinteraction > server.maxidletime as libc::c_long
    {
        if !((1 as libc::c_int & 0xff as libc::c_int) < server.verbosity) {
            _serverLog(
                1 as libc::c_int,
                b"Closing idle client\0" as *const u8 as *const libc::c_char,
            );
        }
        freeClient(c);
        return 1 as libc::c_int;
        ...
        if (*c).flags & ((1 as libc::c_int) << 4 as libc::c_int) as libc::c_ulong != 0 {
            if server.cluster_enabled != 0 {
                if clusterRedirectBlockedClientIfNeeded(c) != 0 {
                    unblockClient(c);
                }
            }
        }
...
#[no_mangle]
pub unsafe extern "C" fn encodeTimeoutKey(
    mut buf: *mut libc::c_uchar,
    mut timeout: uint64_t,
    mut c: *mut client,
) {
    timeout = intrev64(timeout);
    memcpy(
        buf as *mut libc::c_void,
        &mut timeout as *mut uint64_t as *const libc::c_void,
        core::mem::size_of::<uint64_t>() as libc::c_ulong,
    );
    memcpy(
        buf.offset(8 as libc::c_int as isize) as *mut libc::c_void,
        &mut c as *mut *mut client as *const libc::c_void,
        core::mem::size_of::<*mut client>() as libc::c_ulong,
    );
    if core::mem::size_of::<*mut client>() as libc::c_ulong
        == 4 as libc::c_int as libc::c_ulong
    {
        memset(
            buf.offset(12 as libc::c_int as isize) as *mut libc::c_void,
            0 as libc::c_int,
            4 as libc::c_int as libc::c_ulong,
        );
    }
...
#[no_mangle]
pub unsafe extern "C" fn decodeTimeoutKey(
    mut buf: *mut libc::c_uchar,
    mut toptr: *mut uint64_t,
    mut cptr: *mut *mut client,
) {
    memcpy(
        toptr as *mut libc::c_void,
        buf as *const libc::c_void,
        core::mem::size_of::<uint64_t>() as libc::c_ulong,
    );
    *toptr = intrev64(*toptr);
    memcpy(
...
#[no_mangle]
pub unsafe extern "C" fn addClientToTimeoutTable(mut c: *mut client) {
    if (*c).bpop.timeout == 0 as libc::c_int as libc::c_longlong {
        return;
    }
    let mut timeout: uint64_t = (*c).bpop.timeout as uint64_t;
    let mut buf: [libc::c_uchar; 16] = [0; 16];
    encodeTimeoutKey(buf.as_mut_ptr(), timeout, c);
    if raxTryInsert(
    ...
    ) != 0
    {
        (*c)
            ...
            | (1 as libc::c_ulonglong) << 38 as libc::c_int) as uint64_t;
    }
}
#[no_mangle]
pub unsafe extern "C" fn removeClientFromTimeoutTable(mut c: *mut client) {
    if (*c).flags as libc::c_ulonglong & (1 as libc::c_ulonglong) << 38 as libc::c_int
        == 0
    {
        return;
    }
...
#[no_mangle]
pub unsafe extern "C" fn handleBlockedClientsTimeout() {
    if raxSize(server.clients_timeout_table) == 0 as libc::c_int as libc::c_ulong {
        return;
    }
    let mut now: uint64_t = mstime() as uint64_t;
    ...
    raxStart(&mut ri, server.clients_timeout_table);
    raxSeek(
        &mut ri,
    ...
    while raxNext(&mut ri) != 0 {
        let mut timeout: uint64_t = 0;
        let mut c: *mut client = 0 as *mut client;
        decodeTimeoutKey(ri.key, &mut timeout, &mut c);
        if timeout >= now {
            break;
        }
        (*c)
            .flags = ((*c).flags as libc::c_ulonglong
            & !((1 as libc::c_ulonglong) << 38 as libc::c_int)) as uint64_t;
        checkBlockedClientTimeout(c, now as mstime_t);
        raxRemove(
            server.clients_timeout_table,
            ri.key,
            ri.key_len,
...
#[no_mangle]
pub unsafe extern "C" fn getTimeoutFromObjectOrReply(
    mut c: *mut client,
    mut object: *mut robj,
    mut timeout: *mut mstime_t,
    mut unit: libc::c_int,
) -> libc::c_int {
    let mut tval: libc::c_longlong = 0;
    let mut ftval: f64 = 0 as f64;
    if unit == 0 as libc::c_int {
        if getLongDoubleFromObjectOrReply(
            c,
            object,
            &mut ftval,
            b"timeout is not a float or out of range\0" as *const u8
                as *const libc::c_char,
        ) != 0 as libc::c_int
        {
            return -(1 as libc::c_int);
        }
        tval = ((ceill(ftval * (1000.0 as f64))) as i64);
    } else if getLongLongFromObjectOrReply(
        c,
        object,
        &mut tval,
        b"timeout is not an integer or out of range\0" as *const u8
            as *const libc::c_char,
    ) != 0 as libc::c_int
    {
        return -(1 as libc::c_int)
    }
    if tval < 0 as libc::c_int as libc::c_longlong {
        addReplyError(c, b"timeout is negative\0" as *const u8 as *const libc::c_char);
        return -(1 as libc::c_int);
    }
    if tval > 0 as libc::c_int as libc::c_longlong {
        tval += mstime();
