...
unsafe extern "C" fn renderCanvas(mut canvas: *mut lwCanvas) -> sds {
    ...
    while y < (*canvas).height {
        ...
        while x < (*canvas).width {
            let mut color: libc::c_int = lwGetPixel(canvas, x, y);
            let mut ce: *mut libc::c_char = 0 as *mut libc::c_char;
            match color {
                0 => {
                    ce = b"0;30;40m\0" as *const u8 as *const libc::c_char
                        as *mut libc::c_char;
                }
                1 => {
                    ce = b"0;90;100m\0" as *const u8 as *const libc::c_char
                        as *mut libc::c_char;
                }
                2 => {
                    ce = b"0;37;47m\0" as *const u8 as *const libc::c_char
                        as *mut libc::c_char;
                }
                3 => {
                    ce = b"0;97;107m\0" as *const u8 as *const libc::c_char
                        as *mut libc::c_char;
                }
                _ => {
                    ce = b"0;30;40m\0" as *const u8 as *const libc::c_char
            ...
            text = sdscatprintf(
                text,
                b"\x1B[%s \x1B[0m\0" as *const u8 as *const libc::c_char,
                ce,
            );
            x += 1;
        }
...
#[no_mangle]
pub unsafe extern "C" fn generateSkyscraper(
    mut canvas: *mut lwCanvas,
    mut si: *mut skyscraper,
) {
    let mut starty: libc::c_int = (*canvas).height - 1 as libc::c_int;
    let mut endy: libc::c_int = starty - (*si).height + 1 as libc::c_int;
    let mut y: libc::c_int = starty;
    while y >= endy {
        let mut x: libc::c_int = (*si).xoff;
        while x < (*si).xoff + (*si).width {
            if !(y == endy
                && (x <= (*si).xoff + 1 as libc::c_int
                    || x >= (*si).xoff + (*si).width - 2 as libc::c_int))
            {
                let mut color: libc::c_int = (*si).color;
                if (*si).windows != 0 && x > (*si).xoff + 1 as libc::c_int
                    && x < (*si).xoff + (*si).width - 2 as libc::c_int
                    && y > endy + 1 as libc::c_int && y < starty - 1 as libc::c_int
                {
                    let mut relx: libc::c_int = x - ((*si).xoff + 1 as libc::c_int);
                    let mut rely: libc::c_int = y - (endy + 1 as libc::c_int);
                    if relx / 2 as libc::c_int % 2 as libc::c_int != 0
                        && rely % 2 as libc::c_int != 0
                    {
                        loop {
                            color = 1 as libc::c_int + rand() % 2 as libc::c_int;
                            if !(color == (*si).color) {
                                break;
                            }
                        }
                        if relx % 2 as libc::c_int != 0 {
                            color = lwGetPixel(canvas, x - 1 as libc::c_int, y);
                        }
                    }
                }
...
#[no_mangle]
pub unsafe extern "C" fn generateSkyline(mut canvas: *mut lwCanvas) {
    let mut si: skyscraper = skyscraper {
        xoff: 0,
        width: 0,
        height: 0,
        windows: 0,
    ...
    let mut color: libc::c_int = 2 as libc::c_int;
    while color >= 1 as libc::c_int {
        si.color = color;
        let mut offset: libc::c_int = -(10 as libc::c_int);
        while offset < (*canvas).width {
            offset += rand() % 8 as libc::c_int;
            si.xoff = offset;
            si.width = 10 as libc::c_int + rand() % 9 as libc::c_int;
            if color == 2 as libc::c_int {
                si
                    .height = (*canvas).height / 2 as libc::c_int
                    + rand() % (*canvas).height / 2 as libc::c_int;
            } else {
                si
                    .height = (*canvas).height / 2 as libc::c_int
                    + rand() % (*canvas).height / 3 as libc::c_int;
            }
            si.windows = 0 as libc::c_int;
            generateSkyscraper(canvas, &mut si);
            if color == 2 as libc::c_int {
                offset += si.width / 2 as libc::c_int;
            } else {
                offset += si.width + 1 as libc::c_int;
            }
        }
        color -= 1;
    }
    si.color = 0 as libc::c_int;
    let mut offset_0: libc::c_int = -(10 as libc::c_int);
    while offset_0 < (*canvas).width {
        offset_0 += rand() % 8 as libc::c_int;
        si.xoff = offset_0;
        si.width = 5 as libc::c_int + rand() % 14 as libc::c_int;
        if si.width % 4 as libc::c_int != 0 {
            si.width += si.width % 3 as libc::c_int;
        }
        si
            .height = (*canvas).height / 3 as libc::c_int
            + rand() % (*canvas).height / 2 as libc::c_int;
        si.windows = 1 as libc::c_int;
        generateSkyscraper(canvas, &mut si);
        offset_0 += si.width + 5 as libc::c_int;
    }
}
#[no_mangle]
pub unsafe extern "C" fn lolwut6Command(mut c: *mut client) {
    let mut cols: libc::c_long = 80 as libc::c_int as libc::c_long;
    let mut rows: libc::c_long = 20 as libc::c_int as libc::c_long;
    ...
    if rows < 1 as libc::c_int as libc::c_long {
        rows = 1 as libc::c_int as libc::c_long;
    }
    if rows > 1000 as libc::c_int as libc::c_long {
        rows = 1000 as libc::c_int as libc::c_long;
    }
    ...
    rendered = sdscat(
        rendered,
        b"\nDedicated to the 8 bit game developers of past and present.\nOriginal 8 bit image from Plaguemon by hikikomori. Redis ver. \0"
